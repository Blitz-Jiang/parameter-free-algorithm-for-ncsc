...
\]
respectively, with only additive pointwise lower-order terms.  The two
parameter-free guarantees are pointwise asymptotic: for each fixed
problem and initialization they hold below an unknown threshold.
\end{abstract}
...
\begin{enumerate}[label=(\roman*)]
 ...
 O(\Delta_P\kappa^2\sqrt\rho\,\epsilon^{-3/2}).
 \]
 These parameter-free results are pointwise asymptotic rather than
 uniform finite-accuracy certificates; the precise quantifier and its
 limitation are stated explicitly in the corresponding theorems.
\end{enumerate}
...
uses.  The resulting algorithm takes no problem parameter as input
other than the requested tolerance $\epsilon$.
There is, however, an important quantifier in the theorem: for every
fixed problem and initialization there is an unknown threshold
$\epsilon_0>0$ such that the guarantees hold for
$0<\epsilon\le\epsilon_0$.  We therefore call the method
asymptotically parameter-free.  The absence of a uniform, finitely
verifiable threshold is explained in red in
\Cref{subsec:scar-limitation}.
...
have an $L$-Lipschitz gradient, where $0<\mu<L$.  Let $\tau>0$ and
...
at most $2L$.
When $L=\mu$, the step $u_0-L^{-1}\nabla\phi(u_0)$ reaches the
minimizer; we use this direct endpoint argument whenever $\kappa=1$.
\end{theorem}
...
We use a uniform inner tolerance below the outer stationarity scale.
Throughout this section, assume
$0<\epsilon<1$ and set
\begin{equation}\label{eq:pf-tolerances}
 \tau_\epsilon:=\epsilon^2,
 \qquad
 \omega_\epsilon:=\epsilon^2.
\end{equation}

\begin{lemma}[Pointwise asymptotic oracle accuracy]
\label{lem:pf-asymptotic-oracle}
For every fixed payoff satisfying \Cref{ass:ncsc}, there exists
$\epsilon_0>0$ such that the following holds for every
$0<\epsilon\le\epsilon_0$.  If
$\|\nabla_y f(x,y)\|\le\tau_\epsilon$, then, for every
$\sigma\ge\epsilon$,
...
\begin{align}
 ...
 |\widehat P(x,y)-P(x)|
 &\le\frac{\omega_\epsilon}{8}.
       \label{eq:pf-value-error}
\end{align}
\end{subequations}
The same $\epsilon_0$ works uniformly over all $x$, $y$, and
$\sigma\ge\epsilon$.
\end{lemma}
...
computed after an inner-module call with tolerance $\tau_\epsilon$,
and set
...
\end{aligned}
\end{equation}
The Hessian padding makes the quadratic term an upper model once
\eqref{eq:pf-hessian-error} holds.  The much smaller actual gradient
error is $O(\epsilon^4)$; at a safe scale, its worst-case effect on an
interior trial is only $O(\epsilon^{15/2})$ and is absorbed by the
$O(\epsilon^2)$ nonmonotonicity.  Thus the inner solve need not be
...
\begin{algorithm}[H]
\DontPrintSemicolon
\caption{Asymptotically parameter-free residual-module UTR}
\label{alg:pf-residual-utr}
\KwIn{$x_0,y_{-1}$, $0<\epsilon<1$, and an inner module $\mathcal S$
      satisfying \eqref{eq:maxres-interface} without problem
      parameters}
$x\leftarrow x_0$, $\sigma\leftarrow\epsilon$, and
$\eta\leftarrow1/256$\;
$y\leftarrow
 \operatorname{MaxRes}_{\mathcal S}(x_0,y_{-1},\tau_\epsilon)$\;
...
\While{true}{
 ...
 $x^+\leftarrow x+d$\;
 $y^+\leftarrow
  \operatorname{MaxRes}_{\mathcal S}(x^+,y,\tau_\epsilon)$\; compute
 $(\widehat P^+,g^+,H^+)$\;
 \eIf{
  $r<\sqrt h/(4\sigma)$,
  $\widehat P^+\le\widehat P+\omega_\epsilon$, and
  $\|g^+\|\le h/2$
 }{
  accept the trial\;
 }{
  \eIf{
   $r=\sqrt h/(4\sigma)$,
   $\widehat P-\widehat P^+
    \ge\eta(h^{3/2}/\sigma+\lambda r^2)+\omega_\epsilon$, and
   $\|g^+\|\le h/2+\lambda r$
  }{
   accept the trial\;
  }{
...
The target $\tau_\epsilon$ is independent of $\sigma$, so a rejection
changes only the robust trust-region model; the corrected tuple at the
current point remains frozen.  Secant-initialized SCAR from
...
\begin{equation}\label{eq:pf-safe-boundary-decrease}
 ...
 +\frac14\lambda r^2
\end{equation}
for all sufficiently small $\epsilon$, with the threshold depending
only on the fixed problem.
\end{lemma}
...
\begin{equation}\label{eq:pf-absorption-condition}
 \frac{5\bar\sigma\omega_\epsilon}
 {4\eta\epsilon^{3/2}}
...
\begin{theorem}[Asymptotically parameter-free NC--SC complexity]
\label{thm:pf-scar-utr}
Suppose $f$ satisfies \Cref{ass:ncsc}, define
$P(x)=\max_y f(x,y)$, fix
$(x_0,y_{-1})$, and set
\[
 \kappa=\ell/\mu,
 ...
 \qquad \Delta_P=P(x_0)-\inf_xP(x).
\]
Let $\mathcal S$ be any residual module satisfying
\eqref{eq:maxres-interface} whose calls use none of
$\ell,\mu,\rho,\kappa,M_P$, or $\Delta_P$.  There exists a payoff- and
initialization-dependent $\epsilon_0\in(0,1)$ such that, for every
$0<\epsilon\le\epsilon_0$, \Cref{alg:pf-residual-utr} terminates.  The
algorithm itself uses only $\epsilon$ and universal numerical
constants, and its output $\widehat x$ satisfies
...
be the secant-initialized SCAR module in \Cref{cor:scar-inner}.  After
decreasing the pointwise threshold $\epsilon_0$ if necessary, set
\begin{equation}\label{eq:pf-gradient-level}
 G_0:=1+\sqrt{2(1+\kappa)\ell(\Delta_P+1)}.
\end{equation}
If $Q$ denotes the number of robust trust-region subproblems in
\eqref{eq:pf-outer-complexity}, then the total number $N_y$ of
evaluations of $\nabla_yf$ is
\begin{align}
 N_y
 &=O\!\left(
...
The quantifier in \Cref{thm:pf-scar-utr} is pointwise in the problem
and initialization:
the same executable algorithm is used without knowing the parameters,
but the proof supplies an unknown payoff- and initialization-dependent
$\epsilon_0$.
This is weaker than a uniform finite-accuracy theorem with an
a posteriori certificate.  It is nevertheless a conventional
asymptotic complexity result: for every fixed admissible problem, the
outer rate and second-order scale are the same as in the parameter-tuned
analysis once $\epsilon$ enters the stated regime.
\end{remark}

\subsection{A limitation of residual-only parameter-free inner solves}
...
uniform certificate for the value or gradient of
$P(x)=\max_y f(x,y)$ when the strong-concavity modulus is unknown.  The
issue is not merely a loss in the complexity bound: an outer stopping
test based on a residual-only corrected oracle can be false.
...
\[
 ...
 A:=\frac{1-c}{2\pi^2},
\]
and, for $0<r\le1$, define
\begin{equation}\label{eq:scar-counterexample}
 f_r(x,y):=A\tanh(x/A)\bigl(1-\cos(\pi r y)\bigr)
             -\psi(ry-1).
\end{equation}
Because $1-c\le\psi''(t)\le1+c$,
\[
 \partial_{yy}^2 f_r(x,y)
 =A\pi^2r^2\tanh(x/A)\cos(\pi r y)
   -r^2\psi''(ry-1)
 \le-\frac{1-c}{2}r^2.
\]
Thus $f_r(x,\cdot)$ is globally $(1-c)r^2/2$-strongly concave.  Its
second derivatives are uniformly bounded for $0<r\le1$.  Its third
...
$r$.  Moreover,
\[
 P_r(x)\ge f_r(x,1/r)=2A\tanh(x/A)\ge-2A,
\]
so the value function is bounded below.

At $x=0$, the exact maximizer is $y_r^*(0)=1/r$.  At the point $y=0$,
however,
\[
 |\partial_yf_r(0,0)|=r|\psi'(-1)|\longrightarrow0,
 \qquad
 \widehat g_r(0,0)=0,
 \qquad
 \widehat H_r(0,0)=0.
...
\[
 P_r'(0)=\partial_xf_r(0,1/r)=1-\cos\pi=2.
\]
...
\begin{equation}\label{eq:scar-counterexample-value}
 ...
 \approx0.0647,
\end{equation}
independently of $r$.  Therefore a residual tending to zero does not
uniformly control either the corrected value or the corrected gradient
over the present class.  The smoothness constants remain
bounded; only the unknown strong-concavity modulus tends to zero.

Consequently, choosing SCAR as the residual module in
\Cref{alg:ncsc-utr} removes the inner solver's dependence on
$\ell,\mu$, but does not turn its parameter-dependent residual target
and outer scale into a valid uniform fully parameter-free theorem.
Nor does a prescribed finite sequence of positive tolerances repair
the issue: for any finite list $\tau_0,\ldots,\tau_J>0$, choose
$r<\min_j\tau_j/|\psi'(-1)|$ above and initialize every inner call at
$y=0$.  Every
residual test is then already satisfied and can return the same false
outer derivatives.  This observation covers any predetermined finite
number of tolerance-halving phases.  It does not claim that indefinite
...
current strong-convexity guess is below the true $\mu$.  A uniform
finite-accuracy adaptive minimax theorem therefore needs additional
...
problem structure.  The asymptotic result in
\Cref{thm:pf-scar-utr} avoids a false uniform claim by explicitly
placing the unknown threshold $\epsilon_0$ in its quantifiers.
\endgroup
...
As in \Cref{thm:pf-scar-utr}, the result is pointwise asymptotic.  More
precisely, for every fixed payoff and initialization there is an
unknown threshold $\epsilon_0>0$ below which the theorem holds.  The
algorithm itself uses only $\epsilon$ and universal numerical
constants; the precise quantifier and parameter list are stated in
\Cref{thm:adaptive-fixed-radius-scar}.
...
\eqref{eq:scar-original-complexity}.  Every retained line-search
estimate is at most $2L$.
\end{lemma}
...
\begin{lemma}[Working and validation accuracy]
\label{lem:fixed-radius-oracle-accuracy}
Fix a payoff satisfying \Cref{ass:ncsc}, and let
$M_P=4\sqrt2\kappa^3\rho$.  The following statements hold.
\begin{enumerate}[label=(\roman*)]
 \item There exists a problem-dependent $\epsilon_1>0$ such that,
 for every $0<\epsilon\le\epsilon_1$, every
 ...
 \end{equation}
 then there is a problem-dependent $\epsilon_2>0$ such that, for all
 $0<\epsilon\le\epsilon_2$ and all
 ...
 \end{equation}
\end{enumerate}
All thresholds are independent of $x$.
\end{lemma}
...
\begin{lemma}[Safe fixed-radius trial]
\label{lem:adaptive-fixed-radius-safe}
Fix a payoff satisfying \Cref{ass:ncsc} and set
$M_P:=4\sqrt2\,\kappa^3\rho$.  There exists
$\epsilon_3>0$ such that the following holds for every
...
\begin{theorem}[Parameter-free fixed-radius complexity]
...
\[
 ...
 \qquad \Delta_P=P(x_0)-\inf_xP(x).
\]
There exists a payoff- and initialization-dependent
$\epsilon_0\in(0,1)$ such that, for every
...
tolerance lies in the pointwise regime above.
\end{theorem}
...
{\color{red}
\subsection{What the theorem does and does not certify}
\label{subsec:fixed-radius-red-caveats}

The pointwise quantifier in
\Cref{thm:adaptive-fixed-radius-scar} is essential.  The algorithm
uses no unknown problem parameter, but it also cannot observe whether
$\epsilon\le\epsilon_0$.  SCAR certifies a residual only; without a
known lower bound on $\mu$, that residual is not a uniform certificate
for the distance to $y^*(x)$, the inner optimality gap, the corrected
gradient and Hessian estimators, or the corrected value.  The counterexample in
\Cref{subsec:scar-limitation} therefore continues to rule out a
uniform finite-accuracy claim.
...
is discarded before the current tuple is refreshed.  Dropping any of
these rules invalidates the displayed telescope.
...
the trust-region scale or as the Hessian stopping threshold.  The
validator eliminates false termination for each fixed problem once
$\epsilon$ is sufficiently small, but it is not a uniformly
verifiable certificate.  A doubling trick in $\epsilon$ does not by
itself reveal the unknown onset of this asymptotic regime.  A uniform
all-$\epsilon$ theorem would still require additional information such
as a certified lower bound on $\mu$, a certified inner-gap or
value-function interval, compactness with a computable dual gap, or
...
the sharp outer leading order for each fixed problem and initialization
once $\epsilon$ is sufficiently small, but its uniformly accurate inner solves
restore a leading $\log(1/\epsilon)$.  The adaptive fixed-radius
...
parameter-free conclusions remain pointwise asymptotic: the unknown
onset of their small-$\epsilon$ regime is not a uniform
...
after decreasing $\epsilon_0$ so that
$\epsilon^2/\mu\le\mu/\rho$, \Cref{lem:corrected-errors} gives
\begin{align*}
 \|\widehat g-\nabla P\|
...
\end{align*}
All coefficients are finite for the fixed problem.  The first line is
$o(\epsilon)$, the second is $o(\epsilon^{3/2})$, and the third is
$o(\epsilon^2)$.  Since $\sigma\ge\epsilon$, decreasing
$\epsilon_0$ if necessary proves all three inequalities uniformly.
\end{proof}
...
$\sigma\sqrt h\,r^2=h^{3/2}/(16\sigma)$.  Since $h\ge\epsilon$,
the last term of \eqref{eq:pf-safe-decrease} is smaller than
$h^{3/2}/(128\sigma)$ for all sufficiently small $\epsilon$.  This
proves \eqref{eq:pf-safe-boundary-decrease}.
...
\[
 ...
 +\frac{M_P}{2}r^2.
\]
The radius and safe-scale condition bound the four terms other than
$\lambda r$ by, respectively,
$h/4$, $h/128$, $h/256$, and $3h/16$ for every sufficiently small
$\epsilon$.  Their sum is less than $117h/256$, proving
...
$\omega_\epsilon/4$.  Every $k\in\mathcal I_N$ can increase the true
value by at most $5\omega_\epsilon/4$.  Telescoping and using
...
Set $\bar\sigma:=2\sqrt{M_P/6}$.  Decrease the threshold in
\Cref{lem:pf-asymptotic-oracle}, if
necessary, so that
\begin{equation}\label{eq:pf-small-epsilon-conditions}
\begin{gathered}
 \epsilon\le\sqrt{M_P/6},\qquad
 \frac{5\bar\sigma\epsilon^2}
 {4\eta\epsilon^{3/2}}
 \left(1+\frac4{\log2}\right)\le\frac12,\\
 \frac1{\sqrt{M_P/6}\sqrt\epsilon}
...
\end{gathered}
\end{equation}
Each condition holds for all sufficiently small $\epsilon$.  Starting from
$\sigma=\epsilon$, a rejected trial doubles $\sigma$.  The first trial
with $\sigma^2\ge M_P/6$ has $\sigma\le\bar\sigma$.
...
\[
 ...
 =O(\epsilon^{15/2}).
\]
Here we used $h\ge\epsilon$ and the fixed positive safe scale.  This is
at most $3\omega_\epsilon/4$ for sufficiently small $\epsilon$; after
...
\[
 \widehat P-\widehat P^+
 \ge\frac{h^{3/2}}{128\sigma}
     +\frac14\lambda r^2-\frac{\omega_\epsilon}{4}.
\]
Because $h\ge\epsilon$, $\sigma\le\bar\sigma$, and
$\omega_\epsilon=\epsilon^2$, the right-hand side is at least
$\eta(h^{3/2}/\sigma+\lambda r^2)+\omega_\epsilon$ for all sufficiently
small $\epsilon$.  Hence a safe-scale trial either stops or is
...
$\|g_0\|\le\|\nabla P(x_0)\|+\epsilon/256$.  After decreasing
$\epsilon_0$ below a numerical constant, this proves
\eqref{eq:pf-outer-complexity} as stated.

If the explicit stopping test returns, the oracle errors and
$\sigma\le\bar\sigma$ give \eqref{eq:pf-return}.  If the robust
...
\[
 ...
 \succeq0.
\]
Together with \eqref{eq:pf-oracle-bounds}, this gives an even stronger
version of \eqref{eq:pf-return}.  A constant rescaling of the internal
...
The accepted-step proof gives a total interior increase of at most
$5K\epsilon^2/4$.  By \eqref{eq:pf-accepted-bound}, the initial oracle
bound $\|g_0\|\le\|\nabla P(x_0)\|+\epsilon/256$, and
$\epsilon^2\log(1/\epsilon)\to0$, this quantity is at most one after
decreasing $\epsilon_0$ again.  Hence every accepted outer point
satisfies $P(x)-P^*\le\Delta_P+1$.  Since $\nabla P$ is
$(1+\kappa)\ell$-Lipschitz, the standard descent inequality gives
$\|\nabla P(x)\|^2\le2(1+\kappa)\ell(\Delta_P+1)$, and therefore
$h\le G_0$.
...
\[
 \epsilon^2+\ell\|d\|
 \le\epsilon^2+\frac{\ell\sqrt{G_0}}{4\epsilon}.
\]
...
one candidate call per solved subproblem, so summation gives
...
The AR call used in the proof of
\citet[Theorems~4.1 and~5.1]{lan2026optimal}, when called with the
...
$\epsilon\downarrow0$, there is a problem-dependent threshold below
...
$\mathcal A_\epsilon\to\infty$ as $\epsilon\downarrow0$ and the fixed
problem has $M_P>0$.
