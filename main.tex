...
parameter-free SCAR solver and give three fully observable outer
...
A gradient-scaled variant retains these leading bounds by amortizing
its variable tracking work through a cubic-path budget.
These parameter-free guarantees are asymptotic but class-uniform: after
...
\begin{enumerate}[label=(\roman*)]
 ...
 O(\Delta_P\kappa^2\sqrt\rho\,\epsilon^{-3/2}).
 \]
 A gradient-scaled persistent-SCAR variant has the same leading bounds,
 with variable tracking work controlled by a gradient-level potential
 and a cubic-path estimate.
...
oracle reduction.  \Cref{sec:pf-scar-utr,sec:pf-fixed-radius,sec:pf-gradient-scaled}
give the parameter-free SCAR implementations.
...
\begin{assumption}[Certified uniform derivative accuracy]
\label{ass:inexact-oracle}
Fix a target tolerance $\epsilon>0$.  At every iterate, the inexact
oracle returns $g_k\in\R^n$, a symmetric
$H_k\in\R^{n\times n}$, and an observable Hessian-error certificate
$\xi_k\ge0$ satisfying
\begin{equation}\label{eq:generic-oracle-errors}
  \|g_k-\nabla F(x_k)\|\le\frac{\epsilon}{8192},
  \qquad
  \|H_k-\nabla^2F(x_k)\|\le\xi_k
...
\begin{equation}\label{eq:generic-stopping-test}
 \|g_k\|\le\epsilon,
 \qquad
 \lambda_{\min}(H_k)\ge-\sqrt{M\epsilon}+\xi_k.
\end{equation}
...
\begin{aligned}
  ...
  &g_k^Td+\frac12d^T\left(
    H_k+\xi_kI+2\sqrt{\frac M6}\sqrt{h_k}I\right)d,\\
  \operatorname{s.t.}\quad
  &\|d\|\le\frac{\sqrt{h_k}}{4\sqrt{M/6}},
\end{aligned}
\end{equation}
and set $x_{k+1}=x_k+d_k$.  This is a certificate-padded variant of
the additive UTR subproblem in \citet{jiang2026utr}, evaluated with
the derivative estimates $(g_k,H_k)$ and the certificate $\xi_k$;
the trust-region subproblem itself is solved exactly.
The stopping rule is worst-case sharp given only the scalar
certificate $\xi_k$:
$\lambda_{\min}(H_k)\ge-\sqrt{M\epsilon}+\xi_k$ is the weakest
condition on $H_k$ that guarantees
$\lambda_{\min}(\nabla^2F(x_k))\ge-\sqrt{M\epsilon}$ using only
$\|H_k-\nabla^2F(x_k)\|\le\xi_k$.  The matching shift $\xi_kI$
absorbs the same uncertainty in the model.  The remaining clean
coefficients $2$ and $1/4$ reduce the explicit worst-case iteration
constant.  Optimizing decimal coefficients would depend on the
particular counting potential and would not define an intrinsically
...
Fix $x_0$ and $\epsilon>0$, and define
$F^*:=\inf_xF(x)>-\infty$,
$\Delta_F:=F(x_0)-F^*$, and
$h_0:=\max\{\|g_0\|,\epsilon\}$.  Suppose
...
\eqref{eq:generic-subproblem}.  This procedure terminates after at most
\begin{equation}\label{eq:generic-complexity}
  65\sqrt{\frac M6}\,\Delta_F\epsilon^{-3/2}
  +3\log_+\!\frac{h_0}{\epsilon}+2
\end{equation}
nonterminal iterations, equivalently trust-region subproblem solves.
In particular, this is
$O(\Delta_F\sqrt M\,\epsilon^{-3/2}
+\log_+(h_0/\epsilon)+1)$.  Its output $\widehat x$
satisfies
\begin{equation}\label{eq:generic-return}
  \|\nabla F(\widehat x)\|\le\frac{8193}{8192}\epsilon,
  \qquad
  \lambda_{\min}(\nabla^2F(\widehat x))
  \ge-\sqrt{M\epsilon}.
\end{equation}
...
hold and $\inf_xF(x)>-\infty$, and write
$\Delta_F:=F(x_0)-\inf_xF(x)$.  Let $K$ and
$r_k=\|d_k\|$ be generated by
...
\begin{equation}\label{eq:cubic-path}
  ...
  \le4\Delta_F
    +\frac{K}{98\,304\sqrt6}\frac{\epsilon^{3/2}}{\sqrt M}.
\end{equation}
\end{lemma}
...
inner-oracle bound.  The gradient-scaled variant in
\Cref{sec:pf-gradient-scaled} retains the latter bounds with a radius
depending on the corrected gradient.  Keeping the residual interface
common to these constructions makes clear which conclusions use only
correctness of the inner
...
$\|\nabla_yf(x,y)\|\le\tau_{\rm out}$ has the observable certificate
\begin{equation}\label{eq:ncsc-hessian-certificate}
 \xi(x,y):=\frac{(1+\kappa)^2\rho}{\mu}
              \|\nabla_yf(x,y)\|,
\end{equation}
and satisfies
...
\begin{algorithm}[H]
\DontPrintSemicolon
\caption{Certificate-sharp fixed-scale NC--SC UTR}
\label{alg:ncsc-utr}
...
\For{$k=0,1,\ldots$}{
  ...
  $\xi_k\leftarrow(1+\kappa)^2\rho
       \|\nabla_yf(x_k,y_k)\|/\mu$\;
  $h_k\leftarrow\max\{\|g_k\|,\epsilon\}$\;
  \If{$\|g_k\|\le\epsilon$ and
      $\lambda_{\min}(H_k)\ge-\sqrt{M_P\epsilon}+\xi_k$}{
    \Return{$x_k$}\;
  }
...
The residual used to certify the inner solve is already available, so
computing $\xi_k$ requires no additional oracle call.  In the worst
case, \eqref{eq:ncsc-fixed-oracle-errors} makes the curvature threshold
at least as permissive as
\[
 -\left(\sqrt6-\frac1{64}\right)
   \sqrt{\frac{M_P}{6}}\sqrt\epsilon;
\]
when the inner module returns a smaller residual, the tested threshold
moves further toward $-\sqrt{M_P\epsilon}$.  Thus the test uses all of
the curvature slack certified at the current point, rather than the
factor-$1/\sqrt6$ threshold
$-\sqrt{M_P/6}\sqrt\epsilon$ used by the earlier conservative rule.
...
\begin{equation}\label{eq:ncsc-return}
 \|\nabla P(\widehat x)\|\le\frac{8193}{8192}\epsilon,
 \qquad
 \lambda_{\min}(\nabla^2P(\widehat x))
 \ge-\sqrt{M_P\epsilon}.
\end{equation}
...
\end{equation}
More precisely, the method performs $K$ nonterminal outer iterations
and trust-region subproblem solves and $K+1$ corrected-gradient and
Schur-complement constructions.  Here
$y_0:=\operatorname{MaxRes}_{\mathcal S}
(x_0,y_{-1},\tau_{\rm out})$ is the first inner output,
$h_0:=\max\{\|\widehat g(x_0,y_0)\|,\epsilon\}$, and
\begin{equation}\label{eq:ncsc-outer-explicit}
 K\le65\sqrt{\frac{M_P}{6}}\,\Delta_P\epsilon^{-3/2}
      +3\log_+\!\frac{h_0}{\epsilon}+2.
\end{equation}
...
\section{Adaptive gradient-scaled SCAR trust region}
\label{sec:pf-gradient-scaled}

We next allow the trust-region radius to depend on the corrected
gradient while retaining the working accuracy and persistent SCAR
module of \Cref{sec:pf-fixed-radius}.  The number of tracking halvings
now depends on the step length.  A gradient-level potential and a
cubic-path estimate control their cumulative cost.

\subsection{Working accuracy and tracking}

Retain $\mathcal A_\epsilon$, $\sigma_0$, and $c_0$ from
\eqref{eq:fixed-radius-parameters}, and distinguish the inner scale
\[
 r_\epsilon(\sigma):=\frac{\sqrt\epsilon}{4\sigma}
\]
from the outer radius.  A stored tuple is working-accurate when
\begin{equation}\label{eq:gs-working-invariant}
 \|\nabla_yf(x,y)\|\le c_0\ell r_\epsilon(\sigma).
\end{equation}
This is the same invariant as \eqref{eq:working-residual-invariant};
the unknown $\ell$ is used only in its analysis.

\begin{lemma}[Variable-step tracking]
\label{lem:gs-tracking}
Suppose \eqref{eq:gs-working-invariant} holds at $(x,y)$, and let
$d$ be any step with $r=\|d\|$.  Starting from $y$ at $x+d$, apply
\begin{equation}\label{eq:gs-tracking-count}
 ...
 \right)\right\rceil
\end{equation}
successful persistent SCAR halvings.  The returned point satisfies
\eqref{eq:gs-working-invariant} at $x+d$ with the same scale.
At an unchanged $x$, one successful halving restores the invariant
after $\sigma$ is doubled.
\end{lemma}

The following error bounds will also be used at unsafe scales.  Set
\begin{equation}\label{eq:gs-error-constants}
 C_g:=\frac{c_0^2(1+\kappa)\rho\kappa^2}{32},
 \qquad
 C_v:=\frac{7c_0^3\rho\kappa^3}{1536}.
\end{equation}
\begin{lemma}[Working-oracle errors]
\label{lem:gs-working-errors}
Fix the common constants $(\ell,\mu,\rho)$ in \Cref{ass:ncsc}.
For all sufficiently small $\epsilon$, uniformly over the class,
every working-accurate tuple at $\sigma\ge\sigma_0$ satisfies
\begin{equation}\label{eq:gs-working-errors}
 \|\widehat g-\nabla P(x)\|
 \le C_g\frac{\epsilon}{\sigma^2},
 \qquad
 |\widehat P-P(x)|
 \le C_v\frac{\epsilon^{3/2}}{\sigma^3}.
\end{equation}
The safe-scale bounds \eqref{eq:working-safe-errors} and the
validation bounds \eqref{eq:validator-errors} remain valid.
\end{lemma}

\subsection{The gradient-scaled algorithm}

Given a working-accurate tuple, define
\begin{equation}\label{eq:gs-radius}
 h:=\max\{\|\widehat g\|,\epsilon\},
 \qquad R(h,\sigma):=\frac{\sqrt h}{4\sigma},
\end{equation}
and solve
\begin{equation}\label{eq:gs-subproblem}
 \min_{\|d\|\le R(h,\sigma)}\quad
 \widehat g^Td+\frac12d^T
...
\begin{equation}\label{eq:gs-step-quantities}
 r:=\|d\|,
 \qquad W:=\sigma\sqrt h\,r^2,
 \qquad U:=\lambda r^2,
 \qquad T:=\frac{\epsilon^{3/2}}{\sigma}.
\end{equation}
When $h=\epsilon$, the model and radius coincide with
\eqref{eq:adaptive-fixed-radius-subproblem}.  Otherwise the larger
radius is accompanied by the regularization $\sigma\sqrt h I$.
...
$\sigma\leftarrow\mathcal A_\epsilon^{-1}$,
$c_0\leftarrow2^{-12}$, and $x\leftarrow x_0$\;
Choose $z_0\ne y_{-1}$, compute $a_0$ from
\eqref{eq:scar-secant}, and initialize the persistent SCAR state with
$\nu_0=\mathcal M_0=a_0$\;
At $x_0$, apply persistent SCAR halving from $y_{-1}$ until
$\|\nabla_yf(x_0,y)\|\le c_0a_0r_\epsilon(\sigma)$; compute
$(\widehat P,\widehat g,\widehat H)$\;
\While{true}{
 Set $h\leftarrow\max\{\|\widehat g\|,\epsilon\}$, solve
 \eqref{eq:gs-subproblem} globally, and obtain $(d,\lambda)$\;
 Set $x^+\leftarrow x+d$, $r\leftarrow\|d\|$, and form
 $(W,U,T)$ from \eqref{eq:gs-step-quantities}\;
 Starting from $y$, apply $N_{\rm tr}(r,\sigma)$ successful persistent
 SCAR halvings to $q_{x^+}$ and call the result $y^+$\;
 \eIf{$h=\epsilon$ and $r<R(h,\sigma)$}{
  Choose $z^+\ne y^+$ and compute at $x^+$ the secant $a^+$ from
  \eqref{eq:scar-secant}\;
  Continue persistent SCAR halving at $x^+$ until
  $\|\nabla_yf(x^+,y_{\rm v})\|\le c_0a^+\epsilon^{3/2}$;
  compute $(\widehat g_{\rm v},\widehat H_{\rm v})$\;
  \eIf{$\|\widehat g_{\rm v}\|\le\epsilon/2$ and
   $\lambda_{\min}(\widehat H_{\rm v})
    \ge-(21/8)\sigma\sqrt\epsilon$}{
   \Return{$x^+$}\;
   ...
   discard the candidate and validator tuples, set
   $\sigma\leftarrow2\sigma$, apply one successful persistent halving
   at the unchanged $x$, replace $y$ by the returned point, and
   recompute the current corrected tuple\;
  }
 }{
  Compute $(\widehat P^+,\widehat g^+,\widehat H^+)$ at $(x^+,y^+)$\;
  \eIf{$\widehat P-\widehat P^+
        \ge W/8+U/4-T/1024$ and
   ...
   replace $y$ by the returned point, and recompute the current
   corrected tuple\;
  }
 }
}
...
An accepted candidate tuple is stored without further refinement.
The scale is nondecreasing, and the persistent state
$(\nu,\mathcal M)$ is retained after accepted trials, rejected trials,
and failed validations.  As in \Cref{sec:pf-fixed-radius}, only the
curvature estimates are retained across changes of the inner
objective; accelerated momentum is restarted.  The initial residual
query is counted as the first gradient evaluation of the corresponding
AR call, and the terminal residual gradient is reused in the corrected
tuple.  Initialization establishes \eqref{eq:gs-working-invariant}
because $a_0\le\ell$, and \Cref{lem:gs-tracking} preserves it.

\subsection{Safe scale and complexity}

\begin{lemma}[Safe scale for the gradient-scaled method]
\label{lem:gs-safe}
Under \Cref{ass:ncsc}, for all sufficiently small $\epsilon$ with a
threshold depending only on $(\ell,\mu,\rho)$, the following hold.
If $\sigma^2\ge M_P/6$, every ordinary trial of
\Cref{alg:gradient-scaled-scar} satisfies both acceptance tests, and
every trial with $h=\epsilon$ and $r<R(h,\sigma)$ passes validation.
At any scale $\sigma\ge\sigma_0$, successful validation implies
\begin{equation}\label{eq:gs-validator-return}
 \|\nabla P(x^+)\|<\epsilon,
 \qquad
 \lambda_{\min}(\nabla^2P(x^+))
...
holds for every payoff satisfying \Cref{ass:ncsc} with these common
constants and every initialization $(x_0,y_{-1})$.  Set
$\kappa=\ell/\mu$, $M_P=4\sqrt2\,\kappa^3\rho$, and
$\Delta_P=P(x_0)-\inf_xP(x)$.
For every $0<\epsilon\le\epsilon_0$,
\Cref{alg:gradient-scaled-scar} terminates using only $\epsilon$ and
universal numerical constants.  Its output satisfies
...
\begin{align}
 Q&=O\!\left(\Delta_P\sqrt{M_P}\,\epsilon^{-3/2}\right)
     +O_{\rm fixed}(\mathcal A_\epsilon^3),
     \label{eq:gs-outer-complexity}\\
 N_y&=O\!\left(
       \Delta_P\sqrt{\kappa M_P}\,\epsilon^{-3/2}\right)
       +O_{\rm fixed}(\sqrt\kappa\,\mathcal A_\epsilon^3).
     \label{eq:gs-inner-complexity}
\end{align}
\end{subequations}
Here $O_{\rm fixed}$ may depend on the fixed payoff, regularity
constants, and initialization, but not on $\epsilon$.  The leading
outer and inner terms are equivalently
\[
 O(\Delta_P\kappa^{3/2}\sqrt\rho\,\epsilon^{-3/2})
 \quad\text{and}\quad
 O(\Delta_P\kappa^2\sqrt\rho\,\epsilon^{-3/2}).
...
$(\epsilon,\sqrt{M_P\epsilon})$ guarantee whenever that internal
tolerance is in the stated regime.
\end{theorem}

The fixed-radius method uses a constant number of working halvings
per trial.  Here the accepted-step analysis instead uses
\begin{equation}\label{eq:gs-potential}
 \mathcal L_\epsilon(h):=\left\lceil\log_2(h/\epsilon)\right\rceil,
 \qquad
 \Phi:=\widehat P+\frac{\epsilon^{3/2}}{512\sigma}
                         \mathcal L_\epsilon(h).
\end{equation}
Interior acceptance decreases the gradient level, while boundary
acceptance pays for a possible level increase through $\lambda r^2$.
After accounting for scale refreshes, the same budget controls
$\sum \sigma_k^2\|d_k\|^3$ and the total tracking work.
The proofs are given in \Cref{app:gradient-scaled-proofs}.
The class-uniform interpretation and exact-linear-algebra convention
are the same as in \Cref{sec:pf-fixed-radius}.
...
interface.  In the fixed-scale baseline, that residual also provides
the Hessian-error certificate used by the worst-case-sharp stopping
test, so no curvature slack is lost when transferring the estimator
test to the true value-function Hessian.  The Newton-corrected
value-function gradient permits inner
...
initialization, validation, and scale-search costs.  The gradient-scaled
variant retains these leading bounds; its variable tracking work is
amortized by a gradient-level potential and a cubic-path budget.
These parameter-free conclusions are asymptotic but class-uniform: their
...
\[
 g=g_k,\qquad
 B=H_k+\xi_kI+2\sqrt{M/6}\sqrt{h_k}\,I,\qquad
...
\begin{equation}\label{eq:basic-decrease}
 F(x_k)-F(x_{k+1})
 \ge \frac34\sqrt{\frac M6}\sqrt{h_k}\,r_k^2
     +\frac12\lambda_kr_k^2-\frac{\epsilon r_k}{8192}.
\end{equation}
Consequently,
...
\begin{align}
 \lambda_k=0:\quad
 F(x_k)-F(x_{k+1})
 &\ge-\frac{\epsilon^{3/2}}{201\,326\,592\sqrt{M/6}},
...
\begin{equation}\label{eq:gradient-recurrence}
 h_{k+1}\le
 \max\left\{\frac{2849}{4096}h_k
                   +\lambda_kr_k,\epsilon\right\}.
\end{equation}
\end{lemma}
...
most $\epsilon^{3/2}/(201\,326\,592\sqrt{M/6})$.  The finite-prefix argument
...
$3\sqrt{M/6}\sqrt\epsilon\,r^2/4-\epsilon r/8192$ over $r\ge0$
has minimizer $r=\sqrt\epsilon/(12288\sqrt{M/6})$ and minimum
$-\epsilon^{3/2}/(201\,326\,592\sqrt{M/6})$, proving
\eqref{eq:interior-small-increase}.  If $\lambda_k>0$, then
$r_k=\sqrt{h_k}/(4\sqrt{M/6})$, and
\[
  \frac34\sqrt{\frac M6}\sqrt{h_k}\,r_k^2
  -\frac{\epsilon r_k}{8192}
  \ge\left(\frac3{64}-\frac1{32768}\right)
      \frac{h_k^{3/2}}{\sqrt{M/6}}
  =\frac{1535}{32768}\frac{h_k^{3/2}}{\sqrt{M/6}}.
\]
Together with the multiplier term in \eqref{eq:basic-decrease}, this
is \eqref{eq:boundary-decrease}.

The common gradient estimate gives
\begin{align*}
\|g_{k+1}\|
&\le2\sqrt{\frac M6}\sqrt{h_k}\,r_k+2\xi_kr_k
  +\lambda_kr_k+\frac M2r_k^2+\frac{\epsilon}{4096}\\
&\le\left(\frac12+\frac1{128}+\frac3{16}
  +\frac1{4096}\right)h_k+\lambda_kr_k\\
&=\frac{2849}{4096}h_k+\lambda_kr_k.
\end{align*}
Here we used, in order,
$2\sqrt{M/6}\sqrt{h_k}r_k\le h_k/2$,
$2\xi_kr_k\le h_k/128$,
$(M/2)r_k^2\le3h_k/16$,
and $\epsilon/4096\le h_k/4096$.
...
\begin{align}
 |\mathcal B_N|
 &\le\frac{32768}{1535}\sqrt{\frac M6}\Delta_F\epsilon^{-3/2}
      +\frac1{9\,431\,040}|\mathcal I_N|,
      \label{eq:boundary-count}\\
 \sum_{k\in\mathcal B_N}z_k
 &\le8\sqrt{\frac M6}\Delta_F\epsilon^{-3/2}
      +\frac1{25\,165\,824}|\mathcal I_N|.
      \label{eq:z-budget}
\end{align}
\end{subequations}
Furthermore,
...
\[
  \Delta_F
  \ge\frac{1535\epsilon^{3/2}}{32768\sqrt{M/6}}
       |\mathcal B_N|
    +\frac12\sum_{k\in\mathcal B_N}\lambda_kr_k^2
    -\frac{\epsilon^{3/2}}{201\,326\,592\sqrt{M/6}}
       |\mathcal I_N|.
\]
...
\[
 ...
 \le\left[\log\left(\frac{2849}{4096}+z_k\right)\right]_+
 \le z_k.
\]
Indeed, the left-hand side is zero when
$z_k\le1247/4096$; above this threshold,
$z-\log(2849/4096+z)$ is increasing and positive.
For $k\in\mathcal I_N$ with $h_{k+1}>\epsilon$, the quantity
$\log h_k$ decreases by at least $\log(4096/2849)$.  If
...
terminated,
$\lambda_{\min}(H_{k+1})<-\sqrt{M\epsilon}+\xi_{k+1}$.  Therefore
\begin{align*}
 \lambda_{\min}\!\left(
 H_{k+1}+\xi_{k+1}I+2\sqrt{\frac M6}\sqrt\epsilon I\right)
 &< -\sqrt{M\epsilon}+2\xi_{k+1}
       +2\sqrt{\frac M6}\sqrt\epsilon\\
 &\le\left(2+\frac1{32}-\sqrt6\right)
       \sqrt{\frac M6}\sqrt\epsilon<0.
\end{align*}
Applying \eqref{eq:tr-global-optimality} at iteration $k+1$ with
$B=H_{k+1}+\xi_{k+1}I
+2\sqrt{M/6}\sqrt\epsilon I$ therefore forces
...
\begin{align*}
&\left[1-\frac1{9\,431\,040}
  -\frac1{25\,165\,824\log(4096/2849)}\right]
  |\mathcal I_N|\\
&\quad\le
\left(\frac{32768}{1535}
      +\frac8{\log(4096/2849)}\right)
\sqrt{\frac M6}\Delta_F\epsilon^{-3/2}
+1+\frac{\log_+(h_0/\epsilon)}{\log(4096/2849)}.
\end{align*}
...
\[
 ...
 \qquad
 1-\frac1{9\,431\,040}
  -\frac1{25\,165\,824\log(4096/2849)}
  >\frac{9\,999\,997}{10\,000\,000}
\]
and \eqref{eq:boundary-count} now give, by direct substitution,
\begin{equation}\label{eq:finite-prefix-explicit}
 N=|\mathcal I_N|+|\mathcal B_N|
 \le65\sqrt{\frac M6}\Delta_F\epsilon^{-3/2}
      +3\log_+\!\frac{h_0}{\epsilon}+2.
\end{equation}
...
$\|\nabla F(\widehat x)\|\le8193\epsilon/8192$ and, by Weyl's
inequality,
\[
  \lambda_{\min}(\nabla^2F(\widehat x))
  \ge\lambda_{\min}(H_K)-\xi_K
  \ge-\sqrt{M\epsilon},
\]
which proves \eqref{eq:generic-return} without a slack comparison.
\end{proof}
...
\[
 F(x_k)-F(x_{k+1})
 \ge\frac M2r_k^3-\frac{\epsilon r_k}{8192}.
\]
Split the first term equally and maximize
$\epsilon r/8192-Mr^3/4$ over $r\ge0$.  Its maximum is
$\epsilon^{3/2}/(393\,216\sqrt{6M})$, and therefore
\[
 F(x_k)-F(x_{k+1})
 \ge\frac M4r_k^3
 -\frac{\epsilon^{3/2}}{393\,216\sqrt{6M}}.
\]
...
The residual--distance certificate gives
\[
 \|y-y^*(x)\|\le\frac{\|\nabla_yf(x,y)\|}{\mu}.
\]
Consequently, \eqref{eq:corrected-hessian-error} gives
$\|\widehat H-\nabla^2P(x)\|\le\xi(x,y)$.  Moreover,
\eqref{eq:outer-residual-target} gives
\[
 \|y-y^*(x)\|
 \le\frac{\sqrt{M_P\epsilon/6}}
          {64(1+\kappa)^2\rho}.
\]
Substitution in \eqref{eq:corrected-hessian-error} gives the upper
bound on $\xi(x,y)$ in \eqref{eq:ncsc-fixed-oracle-errors}.
Substitution in
...
$M=M_P$.  In particular, \eqref{eq:generic-complexity} gives
\eqref{eq:ncsc-outer-explicit}.  Its logarithmic term uses
...
\begin{align*}
 ...
 &=\frac{M_P^{3/2}}{\epsilon^{3/2}}
   \sum_{k=0}^{K-1}r_k^3\\
 &\le4\Delta_P\sqrt{M_P}\,\epsilon^{-3/2}
 +\frac{K}{98\,304\sqrt6}.
\end{align*}
...
\subsection{Adaptive gradient-scaled SCAR trust region}
\label{app:gradient-scaled-proofs}
...
Strong concavity and \eqref{eq:gs-working-invariant} imply
\[
 e:=\|y-y^*(x)\|
 \le\frac{c_0\kappa\sqrt\epsilon}{4\sigma}
 \le\frac{c_0\kappa}{4}\sqrt\epsilon\,\mathcal A_\epsilon.
\]
For sufficiently small $\epsilon$, uniformly over the class, this is
at most $\mu/\rho$.  The corrected-oracle estimates in
\Cref{sec:ncsc} therefore give
\[
 \|\widehat g-\nabla P(x)\|
 \le\frac{(1+\kappa)\rho}{2}e^2,
 \qquad
 |\widehat P-P(x)|\le\frac{7\rho}{24}e^3.
\]
Substitution yields \eqref{eq:gs-working-errors} with the constants
in \eqref{eq:gs-error-constants}.  The working invariant and the
validation target are unchanged from \Cref{sec:pf-fixed-radius}, so
\Cref{lem:fixed-radius-oracle-accuracy} gives the remaining assertions.
\end{proof}

\begin{proof}[Proof of \Cref{lem:gs-safe}]
Apply \Cref{lem:common-inexact-tr-step} with $F=P$, $M=M_P$,
\[
...
\]
The endpoint errors follow from \eqref{eq:working-safe-errors}.
Since $r\le\sqrt h/(4\sigma)$ and $M_P\le6\sigma^2$,
\[
 \frac{M_P}{6}r^3\le\frac W4.
\]
Thus \eqref{eq:common-function-bound} gives
\[
 P(x)-P(x+d)\ge\frac W4+\frac U2-\frac{\epsilon r}{1024}.
\]
...
If $h=\epsilon$ and $r<R(h,\sigma)$, the model is exactly
\eqref{eq:adaptive-fixed-radius-subproblem}, with an interior
solution.  The interior argument in the proof of
\Cref{lem:adaptive-fixed-radius-safe} applies to the same
working-accurate endpoint tuples and validation target.  It gives
\[
 \|\widehat g_{\rm v}\|\le\frac{458}{1024}\epsilon,
 \qquad
 \lambda_{\min}(\widehat H_{\rm v})
 \ge-\frac{82}{32}\sigma\sqrt\epsilon,
\]
so validation succeeds.  At any generated scale, a successful
validation and \eqref{eq:validator-errors} imply
\[
 \|\nabla P(x^+)\|
...
\begin{equation}\label{eq:gs-layer-growth}
...
$h^+\le\max\{h/2,\epsilon\}$, so \eqref{eq:gs-layer-halving} and
the value test yield
\[
 \Phi-\Phi^+\ge\frac W8-\frac T{1024}+\frac T{512}
 =\frac W8+\frac T{1024}.
\]
This proves \eqref{eq:gs-potential-decrease} in the interior case.

For a boundary step, put $z=\lambda r/h$.  The gradient test gives
$h^+/h\le\max\{1,1/2+z\}$.  By \eqref{eq:gs-layer-growth},
\[
 \mathcal L_\epsilon(h^+)-\mathcal L_\epsilon(h)
...
\begin{equation}\label{eq:gs-budget-parameters}
\begin{aligned}
 E(\sigma)&:=C_v\frac{\epsilon^{3/2}}{\sigma^3},
 &E_0&:=E(\sigma_0),
 &T_0&:=\frac{\epsilon^{3/2}}{\sigma_0},\\
 h_0&:=\max\{\|\widehat g_0\|,\epsilon\},
 &\mathcal L_0&:=\mathcal L_\epsilon(h_0),
 &D_0&:=1+\log_2\left(1+\frac{5C_g}{4\sigma_0^2}\right).
\end{aligned}
\end{equation}
Here $\widehat g_0$ is the corrected gradient after initialization.
Set
\begin{equation}\label{eq:gs-budget}
 B_\epsilon:=
 \Delta_P+\frac{23}{7}E_0+\frac{T_0}{512}(\mathcal L_0+D_0).
\end{equation}

Choose $\epsilon$ small enough that the working-error and safe-scale
lemmas apply and $\sigma_0<\sigma_*:=\sqrt{M_P/6}$.
Every failed trial doubles $\sigma$, and
\Cref{lem:gs-safe} excludes failures at or above $\sigma_*$.
Thus every finite prefix satisfies
\begin{equation}\label{eq:gs-scale-bound}
 J\le\left\lceil\log_2(\sigma_*\mathcal A_\epsilon)\right\rceil,
 \qquad
 \sigma\le\overline\sigma:=2\sigma_*.
\end{equation}

\begin{lemma}[Cumulative accepted-step budget]
\label{lem:gs-budget}
Under the preceding conditions, every finite prefix satisfies
\begin{equation}\label{eq:gs-cumulative-budget}
 \sum_{\rm accepted}
 \left(\frac{W_k}{32}+\frac{T_k}{1024}+\frac{U_k}{16}\right)
 \le B_\epsilon.
\end{equation}
If $K$ denotes its number of accepted steps, then
\begin{equation}\label{eq:gs-path-budget}
 K\le1024\overline\sigma B_\epsilon\epsilon^{-3/2},
 \qquad
 \sum_{\rm accepted}\sigma_k^2r_k^3\le8B_\epsilon.
\end{equation}
Every stored iterate satisfies $P(x)-P^*\le B_\epsilon$, and
...
$2\sigma$.  By \eqref{eq:gs-working-errors}, the corrected-value jump
is at most
\[
...
\]
For the gradient-level term, the two corrected-gradient errors give
\[
...
$P(x_0)+E_0+T_0\mathcal L_0/512$, and every stored potential is at
least $P^*-E_0$.  Telescoping \eqref{eq:gs-potential-decrease} and
paying the two refresh bounds gives
\[
 \sum_{\rm accepted}
 \left(\frac W{32}+\frac T{1024}+\frac U{16}\right)
 \le\Delta_P+2E_0+\frac97E_0
       +\frac{T_0}{512}(\mathcal L_0+D_0)=B_\epsilon.
\]
The same upper bound on the stored potential, together with
$P(x)\le\Phi+E_0$, gives $P(x)-P^*\le B_\epsilon$.

By \eqref{eq:gs-scale-bound},
$T_k\ge\epsilon^{3/2}/\overline\sigma$, which proves the bound on $K$.
The radius constraint also gives
\[
 4\sigma_k^2r_k^3\le\sigma_k\sqrt{h_k}\,r_k^2=W_k.
\]
Combining this with \eqref{eq:gs-cumulative-budget} proves the
cubic-path bound.

Finally,
\[
 E_0=C_v\epsilon^{3/2}\mathcal A_\epsilon^3,
 \qquad T_0=\epsilon^{3/2}\mathcal A_\epsilon.
\]
...
$\mathcal L_0=O_{\rm fixed}(\mathcal A_\epsilon)$, while
$D_0=O_{\rm fixed}(1+\log\mathcal A_\epsilon)$.
Substitution into \eqref{eq:gs-budget} yields
\eqref{eq:gs-budget-asymptotic}.
\end{proof}

\begin{proof}[Proof of \Cref{thm:gradient-scaled-scar}]
Choose $\epsilon_0$ so that
\Cref{lem:gs-working-errors,lem:gs-safe} hold and
$\mathcal A_\epsilon^{-1}<\sqrt{M_P/6}$ for
$0<\epsilon\le\epsilon_0$.  These restrictions depend only on the
common constants $(\ell,\mu,\rho)$.

Each inner sequence is finite by
\Cref{lem:scar-persistent-half} and the positive residual targets.
Every nonterminal outer trial is either accepted or increases the
scale.  The finite-prefix bounds
\eqref{eq:gs-scale-bound,eq:gs-path-budget} bound both possibilities,
so the algorithm terminates.  With $K$ accepted steps and $J$
failures, $Q=K+J+1$, and hence
\[
 Q\le1024\overline\sigma B_\epsilon\epsilon^{-3/2}
       +\left\lceil\log_2(\sigma_*\mathcal A_\epsilon)\right\rceil+1.
\]
Using \eqref{eq:gs-budget-asymptotic} and
$\overline\sigma=2\sqrt{M_P/6}$ gives
\eqref{eq:gs-outer-complexity}.
The output bound \eqref{eq:gs-return} follows from
\eqref{eq:gs-validator-return} and the scale cap.
The tolerance conversion follows from
...
Thus \eqref{eq:gs-path-budget} gives
\begin{align}
 \sum_{\rm accepted}N_{{\rm tr},k}
 &\le16K+\epsilon^{-3/2}
                  \sum_{\rm accepted}\sigma_k^3r_k^3\notag\\
 &\le16K+8\overline\sigma B_\epsilon\epsilon^{-3/2}\notag\\
 &\le16392\overline\sigma B_\epsilon\epsilon^{-3/2}.
...
\]
At every stored iterate, \Cref{lem:gs-budget} and
...
Equations \eqref{eq:gs-budget-asymptotic,eq:gs-accepted-tracking}
therefore imply
\[
 N_{\rm half}
...
\begin{equation}\label{eq:gs-inner-explicit}
 N_y\le(4+2\sqrt{10}\,C_1)\sqrt\kappa
       \bigl(N_{\rm half}+\lceil\log_4\kappa\rceil\bigr)
       +2(J+2).
\end{equation}
