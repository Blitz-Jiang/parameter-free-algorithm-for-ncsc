...
from .utr3 import _CountedInnerProblem
from .utr4 import UTR4
...
class UTR5(UTR4):
    def run(self, x0, y0) -> AlgorithmResult:
        ...
        history = {
            "sigma": [], "grad_norm": [], "lambda_min": [], "value": [],
            "h": [], "radius": [], "working_radius": [], "tracking_halvings": [],
            "step_norm": [], "multiplier": [], "boundary": [],
            "accepted": [], "trial_status": [], "tr_tolerance": [],
            "tr_refined": [], "candidate_value": [], "candidate_grad_norm": [],
            "value_decrease": [], "required_decrease": [], "required_grad_norm": [],
            "value_test": [], "gradient_test": [], "W": [], "U": [], "T": [],
            "gradient_level": [], "potential": [], "cumulative_inner_grad_evals": [],
        ...
        def finish(converged, reason):
            history.update(
                ...
                n_failed_trials=n_trials - n_accepted - n_rejected - int(converged),
                n_value_oracles=n_value_oracles,
                n_derivative_oracles=n_derivative_oracles,
                n_oracle_builds=n_derivative_oracles,
                n_tr_refinements=sum(value is True for value in history["tr_refined"]),
                final_sigma=sigma,
        ...
        try:
            ...
            while n_trials < self.max_iterations:
                phase = f"model at trial {n_trials + 1}"
                grad_norm = torch.linalg.vector_norm(g).item()
                if not math.isfinite(grad_norm):
                    raise RuntimeError("Non-finite corrected gradient norm.")
                h = max(grad_norm, epsilon)
                sqrt_h = math.sqrt(h)
                radius = sqrt_h / (4.0 * sigma)
                working_radius = sqrt_epsilon / (4.0 * sigma)
                if not math.isfinite(working_radius) or working_radius <= 0.0:
                    raise RuntimeError("Invalid working radius.")
                I = torch.eye(H.shape[0], dtype=H.dtype, device=H.device)
                B = H + (sigma * sqrt_h + sigma * sqrt_epsilon / 64.0) * I
                T = epsilon_32 / sigma
                level = math.ceil(math.log2(h) - math.log2(epsilon))
                history["sigma"].append(sigma)
                history["grad_norm"].append(grad_norm)
                history["lambda_min"].append(torch.linalg.eigvalsh(H)[0].item())
                history["value"].append(P_hat)
                history["h"].append(h)
                history["radius"].append(radius)
                history["working_radius"].append(working_radius)
                history["T"].append(T)
                history["gradient_level"].append(level)
                history["potential"].append(P_hat + T * level / 512.0)
                history["cumulative_inner_grad_evals"].append(counted_problem.n_grad_evals)
                for key in (
                    "step_norm", "multiplier", "boundary", "accepted", "tr_tolerance",
                    "tr_refined", "candidate_value", "candidate_grad_norm",
                    "value_decrease", "required_decrease", "required_grad_norm",
                    "value_test", "gradient_test", "W", "U", "tracking_halvings",
                    "validator_secant", "validator_target", "validator_grad_norm",
                    "validator_lambda_min", "validator_residual",
                ):
                ...
                W = sigma * sqrt_h * r ** 2
                U = multiplier * r ** 2
                tracking_halvings = math.ceil(
                    math.log1p(r / (self.c0 * working_radius)) / math.log(2.0)
                )
                ...
                history["W"][-1] = W
                history["U"][-1] = U
                history["tracking_halvings"][-1] = tracking_halvings
                ...
                if h == epsilon and not boundary:
                    ...
                    phase = f"ordinary candidate at trial {n_trials}"
                    P_plus, g_plus, H_plus = self._build_corrected_tuple(x_plus, y_plus, grad_plus)
                    n_value_oracles += 1
                    n_derivative_oracles += 1
                    decrease = P_hat - P_plus
                    required = W / 8.0 + U / 4.0 - T / 1024.0
                    grad_plus_norm = torch.linalg.vector_norm(g_plus).item()
                    required_grad = 0.5 * h + multiplier * r
                    value_test = decrease >= required
                    gradient_test = grad_plus_norm <= required_grad
                    history["candidate_value"][-1] = P_plus
                    history["candidate_grad_norm"][-1] = grad_plus_norm
                    history["value_decrease"][-1] = decrease
                    history["required_decrease"][-1] = required
                    history["required_grad_norm"][-1] = required_grad
                    history["value_test"][-1] = value_test
                    history["gradient_test"][-1] = gradient_test
                    if value_test and gradient_test:
                        ...
                        continue
                    rejection = "boundary_rejected" if boundary else "interior_rejected"
                ...
                if not math.isfinite(sigma):
                    raise RuntimeError("UTR5 scale overflowed.")
